...
**2026-06-25 状态：当前 MVP 已简化，本规格不再作为当前实现输入。** 当前 MVP 不要求精确口型同步、7 类嘴型覆盖、`LipSyncTimeline` 强制路径、文本高亮、波形、延迟校准、上传音频、麦克风录音或音频分析。当前实现只需要讲话状态驱动的张口/闭口规则和俏皮小狗动作。本文以下内容保留为历史探索和未来可选扩展，不能作为当前 Story 的验收要求。
